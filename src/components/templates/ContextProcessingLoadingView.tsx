...
export interface ContextProcessingLoadingViewProps {
  progress?: number;
}
...
const steps: ProcessingStep[] = [
  { number: 1, title: 'Processing Your Responses', description: 'Analyzing your context answers to understand achievements, impact metrics, and domain-specific details.' },
  { number: 2, title: 'Enhancing Bullet Points', description: 'Rewriting resume bullets with quantified results and keywords matched to the job description.' },
  { number: 3, title: 'Finalizing Resume', description: "You'll be automatically redirected to review your enhanced resume once generation is complete." },
];

/** Split-panel context processing loading: resume skeleton (left) + progress card (right) */
export const ContextProcessingLoadingView: React.FC<ContextProcessingLoadingViewProps> = ({
  progress = 22,
}) => {
  return (
      ...
      <div className="w-1/2 p-6 pl-0">
        <div className="bg-white rounded-2xl shadow-sm border border-border-default p-10">
          ...
          <p className="text-base text-text-muted text-center mb-6 max-w-md mx-auto">
            Our AI is incorporating your responses to craft tailored, high-impact bullet points. This may take a moment.
          </p>
